...
// Helper function to load inputs from files
void load_input(const char *filename, uint32_t *input) {
    FILE *file = fopen(filename, "rb");
    if (!file) {
        perror("Failed to open input file");
        exit(EXIT_FAILURE);
    }
    size_t read_count = fread(input, sizeof(uint32_t), MAX_DIGITS, file);
    if (read_count != MAX_DIGITS) {
        fprintf(stderr, "Error: Expected %d elements, but read %zu\n", MAX_DIGITS, read_count);
        fclose(file);
        exit(EXIT_FAILURE);
    }
    fclose(file);
...
__device__ void multiply(uint32_t *a, uint32_t scalar, uint32_t *res) {
    uint64_t carry = 0;
    for (int i = 0; i < MAX_DIGITS; i++) {
        uint64_t product = (uint64_t)a[i] * scalar + carry;
...
int main() {
    uint32_t a[MAX_DIGITS];
    uint32_t b[MAX_DIGITS];

    // Load inputs from binary files
    load_input("a_input.bin", a);
    load_input("b_input.bin", b);
