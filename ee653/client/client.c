...
#define ITERATIONS 1000000 // Number of VDF iterations (changed from T)
...
int main(int argc, char const *argv[]) {
    ...
    if (strncmp(buffer, "PUZZLE", 6) == 0) {
        ...
        mpz_t x, a, b, final_a, final_r, final_P;
        mpz_inits(x, a, b, final_a, final_r, final_P, NULL);
        unsigned long T;
        ...
        if (sscanf(buffer, "PUZZLE %s %s %s %lu", x_str, a_str, b_str, &T) != 4) {
            ...
            mpz_clears(x, a, b, final_a, final_r, final_P, NULL);
            close(sock);
            return 1;
        }
        ...
        compute_vdf(x, a, b, T, final_a, final_r, final_P, &stats);
        clock_gettime(CLOCK_MONOTONIC, &end);
        stats.total_time = (end.tv_sec - start.tv_sec) + (end.tv_nsec - start.tv_nsec) / 1e9;

        // Send solution to server: "SOLUTION r P"
        char solution_msg[4096];
        gmp_sprintf(solution_msg, "SOLUTION %Zd %Zd", final_r, final_P);
